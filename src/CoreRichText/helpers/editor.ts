...
export function getNodeEntryFromSelection(
  ...
  if (!path) return [];
  const nodeTypes = Array.isArray(nodeTypeOrTypes)
    ? nodeTypeOrTypes
    : [nodeTypeOrTypes];
...
export function isList(editor?: PlateEditor) {
  ...
  return element.some(
    (element) =>
      isElement(element) && LIST_TYPES.includes(element.type as BLOCKS),
  );
}
  ...
  const [link] = Array.from(
    getNodeEntries(editor, {
      match: (node) =>
        !isEditor(node) &&
        isElement(node) &&
        LINK_TYPES.includes(node.type as INLINES),
    }),
  );
  return !!link;
}
...
export function getAncestorPathFromSelection(editor: PlateEditor) {
  ...
  return getPathLevels(editor.selection.focus.path).find(
...
export const isAtEndOfTextSelection = (editor: PlateEditor) =>
  ...
  getText(editor, editor.selection?.focus.path).length;
