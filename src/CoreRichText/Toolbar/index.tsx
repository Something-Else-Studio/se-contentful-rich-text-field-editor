...
import {
  ToolbarCodeButton,
  ToolbarDropdownCodeButton,
} from "../plugins/Marks/Code";
...
import {
  ToolbarDropdownSubscriptButton,
  ToolbarSubscriptButton,
} from "../plugins/Marks/Subscript";
import {
...
const Dropdown = ({
  ...
  const isActive =
    editor && dropdownMarks.some((mark) => isMarkActive(editor, mark));
...
const Toolbar = ({ isDisabled }: ToolbarProps) => {
  ...
  const validationInfo = React.useMemo(
    () => getValidationInfo(sdk.field),
    [sdk.field],
  );
  const isListSelected =
    isNodeTypeSelected(editor, BLOCKS.UL_LIST) ||
    isNodeTypeSelected(editor, BLOCKS.OL_LIST);
  ...
  const dropdownItemsAvailable = dropdownMarks.some((mark) =>
    isMarkEnabled(sdk.field, mark),
  );

  const shouldShowDropdown =
    boldItalicUnderlineAvailable && dropdownItemsAvailable;
...
function getValidationInfo(field: FieldAppSDK["field"]): {
  ...
  const someWithValidation = (vals, validation) =>
    vals.some((val) => validation(field, val));

  const isAnyMarkEnabled = someWithValidation(
    Object.values(MARKS),
    isMarkEnabled,
  );
