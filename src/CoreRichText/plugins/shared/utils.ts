...
): link is ResourceLink<"Contentful:Entry"> =>
  !!(link as ResourceLink<"Contentful:Entry">).sys.urn;

export const getLinkEntityId = (
  link: EntityLink | ResourceLink<"Contentful:Entry">,
): string => (isResourceLink(link) ? link.sys.urn : link.sys.id);
