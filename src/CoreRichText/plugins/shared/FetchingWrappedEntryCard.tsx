  ...
  return (
    <WrappedEntryCard
      ...
      useLocalizedEntityStatus={
        sdk.parameters.instance.useLocalizedEntityStatus
      }
      ...
      renderDragHandle={
        !props.isDisabled
          ? (dragHandleProps) => (
              <DragHandle label="drag embedded entry" {...dragHandleProps} />
            )
          : undefined
      }
      releaseLocalesStatusMap={releaseLocalesStatusMap}
...
export const FetchingWrappedEntryCard = (
  props: FetchingWrappedEntryCardProps,
) => {
  ...
  const { releaseAction } = getEntryReleaseStatus(
    props.entryId,
    locales,
    activeRelease,
  );
