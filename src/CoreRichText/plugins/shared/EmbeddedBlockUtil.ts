...
export function getWithEmbeddedBlockEvents(
  ...
  return (editor, { options: { hotkey } }) =>
    (event) => {
      const [, pathToSelectedElement] = getNodeEntryFromSelection(
        editor,
        nodeType,
      );
      ...
      if (hotkey && isHotkey(hotkey, event)) {
        if (nodeType === BLOCKS.EMBEDDED_RESOURCE) {
          selectResourceEntityAndInsert(
            sdk,
            editor,
            editor.tracking.onShortcutAction,
          );
        } else {
...
export async function selectResourceEntityAndInsert(
  ...
  const { field, dialogs } = sdk;
  const config = newResourceEntitySelectorConfigFromRichTextField(
    field,
    BLOCKS.EMBEDDED_RESOURCE,
  );
...
function insertBlock(editor: PlateEditor, nodeType: string, entity) {
  ...
  const hasText =
    editor.selection && !!getText(editor, editor.selection.focus.path);
