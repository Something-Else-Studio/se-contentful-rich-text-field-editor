...
const resolvableEntryUrn =
  "crn:contentful:::content:spaces/space-id/entries/linked-entry-urn";
const unknownEntryUrn =
  "crn:contentful:::content:spaces/space-id/entries/unknown-entry-urn";
  ...
  sdk = {
    ...
    cmaAdapter: createFakeCMAAdapter({
      ...
      Locale: {
        getMany: jest
          .fn()
          .mockResolvedValue({ items: [{ default: true, code: "en" }] }),
      },
      ScheduledAction: {
...
test("renders unsupported entity card when unsupported link is passed", async () => {
  ...
  await waitFor(() =>
    expect(getByText("Unsupported API information")).toBeDefined(),
  );
});
...
test("renders missing entity card when unknown error is returned", async () => {
  ...
  await waitFor(() =>
    expect(getByTestId("cf-ui-missing-entity-card")).toBeDefined(),
  );
});
