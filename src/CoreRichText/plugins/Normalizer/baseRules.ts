...
const isTextContainer = (node: Element) =>
  TEXT_CONTAINERS.includes(node.type as BLOCKS);
...
export const baseRules: Required<NormalizerRule>[] = [
  {
    ...
    validNode: (editor, [, path]) => {
      ...
      return (
        !!parent &&
        (isTextContainer(parent) || isInline(parent) || editor.isVoid(parent))
      );
    },
  ...
  },
  {
    ...
    validNode: (_editor, [node]) =>
      typeof node.text === "string" && !node.text.includes("\r"),
    transform: (editor, [node, path]) => {
      return insertText(editor, (node.text as string).replace(/\r/g, ""), {
        at: path,
      });
    },
  },
