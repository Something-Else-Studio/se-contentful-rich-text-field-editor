...
function getCommandPermissions(sdk: FieldAppSDK, editor: PlateEditor) {
  ...
  const { inlineAllowed, entriesAllowed, assetsAllowed } =
    isCommandPromptPluginEnabled(sdk);
...
export const useCommands = (
  ...
  const { inlineAllowed, entriesAllowed, assetsAllowed } =
    getCommandPermissions(sdk, editor);
  const allowedContentTypesFromValidation =
    getAllowedContentTypesFromValidation(sdk.field.validations);

  const filterContentTypesByValidation = (type) =>
    contentTypes.filter(
      (contentType) =>
        allowedContentTypesFromValidation[type]?.[contentType.sys.id],
    );

  const filteredBlockContentTypes = filterContentTypesByValidation(
    BLOCKS.EMBEDDED_ENTRY,
  );
  const filteredInlineContentTypes = filterContentTypesByValidation(
  ...
  const relevantContentTypes = contentTypes.filter(
    (ct) =>
      blockContentTypesToUse.includes(ct) ||
      inlineContentTypesToUse.includes(ct),
  );
