...
export function replaceEmptyParagraphWithTable(editor: PlateEditor) {
  ...
  const isPreviousNodeTextEmpty = isAncestorEmpty(
    editor,
    previousNode as Ancestor,
  );
 ...
 */
export const getNoOfMissingTableCellsInRow = (
  editor: PlateEditor,
  rowEntry: NodeEntry,
) => {
  ...
  const tableWidth = Math.max(
    ...getChildren(parent).map((entry) => getChildren(entry).length),
  );
