...
export const moveListItemDown = (
  ...
  if (previousSiblingItem) {
    ...
    const subList = Array.from(getNodeChildren(editor, previousPath)).find(
      ([n, path]) => match(n, path, { type: getListTypes(editor) }),
    ) as NodeEntry | undefined;

    const newPath = getNextPath(
      getLastChildPath(subList ?? previousSiblingItem),
    );
    ...
    withoutNormalizing(editor, () => {
      if (!subList) {
        ...
        wrapNodes(
          editor,
          { type: listNode.type, children: [], data: {} } as Element,
          {
            at: listItemPath,
          },
        );
