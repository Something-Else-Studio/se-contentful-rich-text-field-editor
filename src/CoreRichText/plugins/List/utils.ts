...
export const hasListAsDirectParent = (
  editor: PlateEditor,
  [, path]: NodeEntry,
) => {
...
const getNearestListAncestor = (editor: PlateEditor, path: Path) => {
  return (
    getAboveNode(editor, { at: path, mode: "lowest", match: isList }) || []
  );
};
 ...
 */
export const normalizeOrphanedListItem = (
  editor: PlateEditor,
  [, path]: NodeEntry,
) => {
...
export const firstNodeIsNotList = (
  _editor: PlateEditor,
...
export const insertParagraphAsChild = (
  editor: PlateEditor,
  [, path]: NodeEntry,
) => {
  insertNodes(
...
export const replaceNodeWithListItems = (
  editor: PlateEditor,
