...
const isEntryAnchor = (element: HTMLElement) =>
  element.nodeName === "A" &&
  element.getAttribute("data-link-type") === "Entry";

const isAssetAnchor = (element: HTMLElement) =>
  element.nodeName === "A" &&
  element.getAttribute("data-link-type") === "Asset";
...
const getNodeOfType =
  (type: INLINES) => (el: HTMLElement, node: AnyObject) => ({
    type,
    ...
    plugins: [
      // URL Hyperlink
      {
        ...
        deserializeHtml: {
          ...
          query: (el) =>
            isAnchor(el) && !(isEntryAnchor(el) || isAssetAnchor(el)),
