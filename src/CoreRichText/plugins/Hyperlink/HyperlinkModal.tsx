...
export function HyperlinkModal(props: HyperlinkModalProps) {
  ...
  const [linkType, setLinkType] = React.useState(
    props.linkType ?? defaultLinkType,
  );
  const [linkTarget, setLinkTarget] = React.useState(props.linkTarget ?? "");
  const [linkEntity, setLinkEntity] = React.useState<
  ...
  function isResourceLink(
    link: Link | ResourceLink | null,
  ): link is ResourceLink {
  ...
  async function selectEntry() {
    const options = {
      ...
      contentTypes: getLinkedContentTypeIdsForNodeType(
        props.sdk.field,
  ...
  async function selectResourceEntry() {
    const options = {
      allowedResources: getAllowedResourcesForNodeType(
        props.sdk.field,
    ...
    const entityLink =
      await props.sdk.dialogs.selectSingleResourceEntity(options);
    if (entityLink) {
      setLinkTarget("");
...
export async function addOrEditLink(
  ...
  let linkTarget;
  let linkEntity;
  const [node, path] = getNodeEntryFromSelection(
    editor,
    LINK_TYPES,
    targetPath,
  );
  ...
  const selectionAfterFocus =
    targetPath ??
    (selectionBeforeBlur as NonNullable<typeof selectionBeforeBlur>);

  const currentLinkText =
    linkText || (editor.selection ? getText(editor, editor.selection) : "");
  const isEditing = Boolean(node && path);

  logAction(
    isEditing ? "openEditHyperlinkDialog" : "openCreateHyperlinkDialog",
  );
  ...
  if (!data) {
    focus(editor);
    logAction(
      isEditing ? "cancelEditHyperlinkDialog" : "cancelCreateHyperlinkDialog",
    );
    return;
  }
