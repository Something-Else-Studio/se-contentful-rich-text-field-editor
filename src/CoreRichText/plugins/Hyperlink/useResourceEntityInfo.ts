...
export function useResourceEntityInfo({
  onEntityFetchComplete,
  target,
}: ResourceEntityInfoProps) {
  const { data, error, status } = useResource<Entry>(
    target.sys.linkType,
    target.sys.urn,
  );
  ...
  const title =
    truncateTitle(
      data.resource.fields[data.contentType.displayField]?.[
        data.defaultLocaleCode
      ],
      40,
    ) || "Untitled";
